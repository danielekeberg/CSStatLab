            ...
            <div className="max-h-100 overflow-y-auto text-sm opacity-70">
                <table className="w-full">
                    <tbody className="text-center">
                        {matches.map((m) => (
                            <tr key={m.match_id} className={`p-2 grid grid-cols-7 md:grid-cols-3 border-b border-neutral-500/20 w-full ${m.has_banned_player && 'bg-red-900/40'}`}>
                                <td className="flex items-center gap-3">
                                    ...
                                    <div>{m.map_name ? <div className="flex items-center gap-2"><img className="h-5" src={`https://leetify.com/assets/images/map-icons/${m.map_name}.svg`}/><p className="hidden md:block">{m.map_name.slice(3,4).toUpperCase()}{m.map_name.slice(4)}</p></div> : <p>{m.map_name ? m.map_name : 'No data'}</p>}</div>
                                </td>
