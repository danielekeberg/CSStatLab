                    ...
                    <tbody className="text-center">
                        {matches.map((m) => (
                            <tr key={m.match_id} className={`p-2 grid grid-cols-7 md:grid-cols-8 border-b border-neutral-500/20 w-full ${m.has_banned_player && 'bg-red-900/40'}`}>
                                ...
                                <td className={`font-bold text-left ${m.rounds_won > m.rounds_lost && 'text-green-500'} ${m.rounds_won < m.rounds_lost && 'text-red-500'} ${m.rounds_won === m.rounds_lost && 'text-{#eae8e0]'}}`}>{m.rounds_won > m.rounds_lost ? <p>{m.rounds_won}:{m.rounds_lost}</p> : <p>{m.rounds_lost}:{m.rounds_won}</p>}</td>
