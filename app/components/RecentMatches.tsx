...
export default function RecentMatches({ matches }: { matches: any[]}) {
    return(
        <div className="mt-10 p-5">
            ...
            <div className="w-full p-2 grid grid-cols-7 md:grid-cols-10 text-sm opacity-70 font-bold border-b border-neutral-500 text-center">
                <p className="text-left">Map</p>
                <p></p>
                <p className="text-left hidden md:block">Mode</p>
            ...
            <div className="max-h-56 overflow-y-auto text-sm opacity-70">
                <table className="w-full">
                    <tbody className="text-center">
                        {matches.map((m) => (
                            <tr key={m.match_id} className={`p-2 grid grid-cols-7 md:grid-cols-10 border-b border-neutral-500/20 w-full ${m.has_banned_player && 'bg-red-900/40'}`}>
                                ...
                                <td className="text-left hidden md:block">{m.mode === "matchmaking_competitive" && "Competitive"}{m.mode === "faceit" && "FACEIT"}{m.mode === "matchmaking" && "Premier"}{m.mode === "hltv" && "HLTV"}</td>
                                <td className={`font-bold text-center ${m.rounds_won > m.rounds_lost && 'text-green-500'} ${m.rounds_won < m.rounds_lost && 'text-red-500'} ${m.rounds_won === m.rounds_lost && 'text-{#eae8e0]'}}`}>{m.rounds_won > m.rounds_lost ? <p>{m.rounds_won}:{m.rounds_lost}</p> : <p>{m.rounds_lost}:{m.rounds_won}</p>}</td>
                                ...
                                <td><AimScore
                                    accuracy={m.accuracy}
                                    accuracyEnemySpotted={m.accuracy_enemy_spotted}
                                    accuracyHead={m.accuracy_head}
                                    reactionTimeMs={m.reaction_time_ms}
                                    preaim={m.preaim}
                                    sprayAccuracy={m.spray_accuracy}
