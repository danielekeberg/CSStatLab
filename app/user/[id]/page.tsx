    ...
    useEffect(() => {
        async function fetchPlayer() {
            try {
                ...
                const grouped: Record<string, {now: number[], kd: number[] }> = {};
                for(const match of matchDetails) {
                  const dayKey = match.finished_at.slice(0,10);
                  const dayKeyLong = match.finished_at.slice(0,30);
                  if(!grouped[dayKey]) {
                    grouped[dayKey] = {now: [], kd: []};
                  }
            ...
            <div className="flex gap-5">
                ...
                <div>
                    <div className="flex items-center gap-2">
                        <img src={`https://flagsapi.com/NO/flat/64.png`} className={`h-6 ${faceit.length <= 1 ? 'hidden' : 'block'}`} />
        ...
        <div className="grid grid-cols-5 gap-5 my-10">
            ...
            <div className="bg-neutral-900 p-3 rounded">
                ...
                <h5 className="text-xl font-bold my-2">
                    {(() => {
                        const recent = player?.recent_matches?.slice(0, 30) ?? [];
            ...
            <div className="w-full h-80">
                <div className="mb-5 flex justify-between">
                    <div>
                        ...
                        <p className="text-neutral-400 text-sm">Rating over last 8 days</p>
                    </div>
